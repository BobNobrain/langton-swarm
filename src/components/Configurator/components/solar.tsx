import { Show, type Component } from 'solid-js';
import { Badge } from '@/components/Badge/Badge';
import { Button } from '@/components/Button/Button';
import { ListItem } from '@/components/List/List';
import { Select, type SelectCustomOptionProps, type SelectOption } from '@/components/Select/Select';
import { BatteryConfiguration, getMaxSolarPower, SolarConfiguration } from '@/game/config';
import { Symbols } from '@/lib/ascii';
import type { UnitComponent } from './types';
import { findValue } from './utils';
import { UnitComponentWarning } from './warning';
import styles from '../Configurator.module.css';

const SOLAR_OPTIONS: SelectOption<SolarConfiguration>[] = [
    // { text: 'None', value: null },
    { text: 'Crude photoelements', value: SolarConfiguration.Tier1Cheap },
    { text: 'Simple solar panel', value: SolarConfiguration.Tier1Regular },
    { text: 'Large solar array', value: SolarConfiguration.Tier2Mobile },
    { text: 'Solar power plant', value: SolarConfiguration.Tier2Static },
];

const NO_SOLAR_DESCRIPTION = 'This unit will not recharge its batteries using the sun';
const SOLAR_DESCRIPTIONS: Record<SolarConfiguration, string> = {
    [SolarConfiguration.Tier1Cheap]: 'A cheap but viable option for units with low energy consumption',
    [SolarConfiguration.Tier1Regular]: 'A reliable source of energy – when the weather permits it',
    [SolarConfiguration.Tier2Mobile]: 'A big array of solar panels that will cover most of your needs',
    [SolarConfiguration.Tier2Static]:
        'A solar array so big that it will satisfy the hungries unit – at the cost of being very massive',
};

const BADGE_CLS: Record<SolarConfiguration | '', string> = {
    '': styles.badgeTier0,
    [SolarConfiguration.Tier1Cheap]: styles.badgeTier1,
    [SolarConfiguration.Tier1Regular]: styles.badgeTier1,
    [SolarConfiguration.Tier2Mobile]: styles.badgeTier2,
    [SolarConfiguration.Tier2Static]: styles.badgeTier2,
};

const SOLAR_TIERS: Record<SolarConfiguration, 1 | 2> = {
    [SolarConfiguration.Tier1Cheap]: 1,
    [SolarConfiguration.Tier1Regular]: 1,
    [SolarConfiguration.Tier2Mobile]: 2,
    [SolarConfiguration.Tier2Static]: 2,
};

const CustomSolarOption: Component<SelectCustomOptionProps<SolarConfiguration | null>> = (props) => {
    return (
        <ListItem
            selected={props.selected}
            onClick={props.onClick}
            right={
                <Badge style="solid" icon={Symbols.LightShade} class={BADGE_CLS[props.value ?? '']}>
                    {getMaxSolarPower({ solar: props.value ?? undefined })}
                </Badge>
            }
            bottom={
                <div class={styles.optionDescription}>
                    {props.value === null ? NO_SOLAR_DESCRIPTION : SOLAR_DESCRIPTIONS[props.value]}
                </div>
            }
        >
            <span class={styles.optionName}>{props.text}</span>
        </ListItem>
    );
};

export const SolarUnitComponent: UnitComponent = {
    cls: styles.componentSolar,
    icon: Symbols.LightShade,
    name: 'Solar',
    description: ({ solar }) => (solar ? SOLAR_DESCRIPTIONS[solar] : ''),
    initial: { solar: SolarConfiguration.Tier1Cheap },
    remove: { solar: undefined },
    isolate: ({ solar }) => ({ solar }),
    isPresent: ({ solar }) => Boolean(solar),
    getTier: ({ solar }) => (solar ? SOLAR_TIERS[solar] : 0),
    inputRenderer: (props) => {
        return (
            <Select
                options={SOLAR_OPTIONS}
                value={findValue(SOLAR_OPTIONS, props.config.solar)}
                onUpdate={(option) => {
                    props.onUpdate({ solar: option.value ?? undefined });
                }}
                popupOpening="manual"
                dark
                sidewaySwitchable
                customOption={CustomSolarOption}
            />
        );
    },
    warningsRenderer: (props) => {
        return (
            <Show when={!props.config.battery}>
                <UnitComponentWarning
                    actions={
                        <Button onClick={() => props.onUpdate({ battery: BatteryConfiguration.Tier1Small })}>
                            Add Battery
                        </Button>
                    }
                >
                    This configuration does not include a battery – solar panels are useless!
                </UnitComponentWarning>
            </Show>
        );
    },
};
